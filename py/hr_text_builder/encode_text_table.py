...
class encode_text_table(object):
    ...
    def _cluster_into_rows(self, text_table):
        ...
        for i in range(len(YX1_sorted[:,0]) - 1):
            ...
            if YX1_sorted[i+1,0] - YX1_sorted[i,0] > \
               self._row_cluster_threshold * avg_boundingPoly_height:
                y1_cluster_label += 1
    ...
    def run(self, text_table):
        ...
        if text_table == 'Bad JSON File':
            ...
            for i in range(len(text_table)):
                if text_table[i][6] == 0:
                    if text_table[i][2] <= self._indent_base:
                        text_table[i].append(0)
                    elif text_table[i][2] > self._indent_base and \
                         text_table[i][2] <= 2 * self._indent_base:
                        text_table[i].append(1)
                    if text_table[i][2] > 2 * self._indent_base and \
                       text_table[i][2] < 3 * self._indent_base:
