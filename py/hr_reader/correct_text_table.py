def correct_text_table(text_table):
    ...
    # Correct for ditto signs that have been given their own entry
    for i in range(len(text_table)):
        for j in range(len(text_table[i])-1):
            item0 = text_table[i][j]
            item1 = text_table[i][j+1]
            if item0[0] == '"' and item0[2] == 0:
                if item0[1] != item1[1]:
                    text_table[i][j][1] = text_table[i][j+1][1]
            item0 = text_table[i][j]
            item1 = text_table[i][j+1]
            if item0[1] == item1[1]:
                text_table[i][j+1][2] = text_table[i][j][2] + 1
